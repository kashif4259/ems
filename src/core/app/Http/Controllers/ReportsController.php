...
class ReportsController extends Controller
{
	...
	public function gettransactions(Request $request){
		...
		\Log::info('SQL Query:', ['query' => $data->toSql(), 'bindings' => $data->getBindings()]);
	...
	public function getaccounttransaction(Request $request) {
		// Calculate initial balance, total income, and total expense
		...
		if ($request->filled('account')) {
			...
			$accountDetails = DB::table('account')->where('accountid', $accountid)->first();
			if($accountDetails)
			{
				$initialBalance = $accountDetails->balance;
				$totalIncome = DB::table('transaction')->where('type', 1)->where('accountid', $accountid)->sum('amount');
				$totalExpense = DB::table('transaction')->where('type', 2)->where('accountid', $accountid)->sum('amount');
			}
		}

	
		// Your existing query to get transactions
		$data = DB::table('transaction')
			->select([
				'transaction.*',
				'category.name as category',
				'subcategory.name as subcategory',
				DB::raw("IFNULL(a.amount, '0') as income"),
				DB::raw("IFNULL(b.amount, '0') as expense"),
				DB::raw(" -
		...
		// Pass initial balance, total income, and total expense to the view
		return Datatables::of($data)
			->with([
			...
			->addColumn('running_balance', function($single) {
				return number_format($single->running_balance, 2); // Adjust as needed
			})
			->filter(function ($query) use ($request) {
				if ($request->filled('names')) {
					$query->where('transaction.name', 'like', "%{$request->get('names')}%");
	...
	public function getuserbalancereportdata(Request $request)
	{
		...
		->addColumn('remainingbalance', function($single){
			$setting = DB::table('settings')->where('settingsid','1')->get();
			$remainingBalance = $single->accountbalance + ($single->totalincome - $single->totalexpense);
	...
	public function generalReport()
	{
		$data = $this->globaldata;
		$users = App\User::where('status', 'Active')->get(['userid', 'name']);
		$accounts = App\AccountModel::get();
		
		if (Auth::user()->isrole('12')){
            return view('reports.generalreport')->with([
				'data' => $data,
				'users' => $users,
				'accounts' => $accounts
			]);
	...
	public function getDataForGeneralReport(Request $request) {

	$select = [
		'users.name as username',
		'account.name as accountname',
		DB::raw('SUM(CASE WHEN transaction.type = "1" THEN transaction.amount ELSE 0 END) as total_income'),
		DB::raw('SUM(CASE WHEN transaction.type = "2" THEN transaction.amount ELSE 0 END) as total_expense'),
		'account.balance as remaining_balance'
	];
	...
	if ($request->filled('fromdate') && $request->filled('todate')) {
		$select[] = DB::raw('MIN(transaction.transactiondate) as start_date');
		$select[] = DB::raw('MAX(transaction.transactiondate) as end_date');
		$includeStartDate = true;
		$includeEndDate = true;
	} else if ($request->filled('fromdate')) {
		$select[] = DB::raw('MIN(transaction.transactiondate) as start_date');
		$includeStartDate = true;
	} else if ($request->filled('todate')) {
		$select[] = DB::raw('MAX(transaction.transactiondate) as end_date');
		$includeEndDate = true;
	}

	\Log::info('Columns selected: ', $select);
	...
	if ($request->filled('user')) {
		$data->where('users.userid', 'like', "%{$request->get('user')}%");
	}
	if ($request->filled('account')) {
		$data->where('account.accountid', 'like', "%{$request->get('account')}%");
	}
	if ($request->filled('category')) {
		$data->where('category.categoryid', 'like', "%{$request->get('category')}%");
	}
	if ($request->filled('subcategory')) {
		$data->where('subcategory.subcategoryid', 'like', "%{$request->get('subcategory')}%");
	}
	if ($request->filled('fromdate') && $request->filled('todate')) {
		$data->whereBetween('transaction.transactiondate', [$request->get('fromdate'), $request->get('todate')]);
	} else if ($request->filled('fromdate')) {
		$data->where('transaction.transactiondate', '>=', $request->get('fromdate'));
	} else if ($request->filled('todate')) {
		$data->where('transaction.transactiondate', '<=', $request->get('todate'));
	}

	$firstResult = $data->first();
	if (!$firstResult || (
		$firstResult->username === null && $firstResult->accountname === null &&
		(!$includeStartDate || $firstResult->start_date === null) &&
		(!$includeEndDate || $firstResult->end_date === null) &&
		$firstResult->total_income === null && $firstResult->total_expense === null &&
		$firstResult->remaining_balance === null)) {
		$data = [];
		// return response()->json(['message' => 'No data found'], 404);
	}

	return Datatables::of($data)
		->addColumn('total_income', function($single) {
			return number_format($single->total_income, 2);
		})
		->addColumn('total_expense', function($single) {
			return number_format($single->total_expense, 2);
		})
		->addColumn('remaining_balance', function($single) {
			$balance = $single->remaining_balance;
			$balance = ($balance - $single->total_expense);
			$balance = ($balance + $single->total_income);
			return number_format($balance, 2);
		})
		->addColumn('start_date', function($single) use ($includeStartDate) {
			if ($includeStartDate && $single->start_date !== null) {
				$setting = DB::table('settings')->where('settingsid', '1')->first();
				return date($setting->dateformat, strtotime($single->start_date));
			}
			return null;
		})
		->addColumn('end_date', function($single) use ($includeEndDate) {
			if ($includeEndDate && $single->end_date !== null) {
				$setting = DB::table('settings')->where('settingsid', '1')->first();
				return date($setting->dateformat, strtotime($single->end_date));
			}
			return null;
		})
		->addColumn('categoryname', function($single) use ($includeCategory) {
			if ($includeCategory && $single->categoryname !== null) {
				return $single->categoryname;
			}
			return null;
		})
		->addColumn('subcategoryname', function($single) use ($includeSubcategory) {
			if ($includeSubcategory && $single->subcategoryname !== null) {
				return $single->subcategoryname;
			}
			return null;
		})
