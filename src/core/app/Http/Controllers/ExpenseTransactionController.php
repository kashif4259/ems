...
class ExpenseTransactionController extends Controller
{
	...
	public function dashboard(){
		$data = $this->globaldata;
    	if (Auth::user()->isrole('4')){
			return view( 'expense.index' )->with('data', $data);
		} else{
			 return redirect('home')->with('data', $data);
		}
    }
	...
	public function upcomingexpense() {
		...
		if (Auth::user()->isrole('20')){
			return view('expense.upcomingexpense')->with('data', $data);
		} else{
			 return redirect('home')->with('data', $data);
		}
	...
	public function totalupcoming() {
		...
		if(Auth::user()['role'] == 'Staff'){
			$user_id = Auth::user()['userid'];
			$totalbalance->where('transaction.userid', '=', $user_id);
		}
		$totalbalance = $totalbalance->where('type', '=', '4')
		->get();
	...
	public function getdata(){
		...
		$data = DB::table('transaction')
			...
			$data = $data->where('transaction.type','2')
            ->get();
	...
	public function getdataupcoming(){
		...
		->addColumn('action', function ($accountsingle) {
				...
				if($accountsingle->file!=''){
					return '<a href="#" id="btnpay" customdata='.$accountsingle->transactionid.' class="text-warning" data-toggle="modal" data-target="#pay"><i data-toggle="tooltip" data-placement="top" title='. trans('lang.pay_bill').' class="ti-wallet"></i></a>
    ...
    public function saveexpense(Request $request){
		...
		if($request->hasFile('expensefile')) {
			$this->validate($request, [
            'expensefile' => 'mimes:jpeg,png,jpg,pdf|max:2048'
			],$message);
		
			$expensefilename 	= str_replace(' ', '-', $request->file('expensefile')->getClientOriginalName());
