...
class UserController extends Controller
{
	...
	public function profile() {
		...
		return view( 'settings.profile.index' )->with('data', $data);
	}
	//get all users view
	...
	if (Auth::user()->isrole('17')){
           return view('settings.profile.users')->with('data', $data);
        } else{
	...
	public function getuser(){
		...
		{
		    $data = DB::table('users')->where('role','<>','Administrator')->where('userid', '<>', Auth::id())->get();
		...
		->addColumn('action', function ($accountsingle) {
			...
			return '<a  href="#" id="btnedit" customdata='.$accountsingle->userid.' class="text-blue-sky" data-toggle="modal" data-target="#edit"><i data-toggle="tooltip" data-placement="top" title='. trans('lang.edit').' class="ti-pencil"></i></a>&nbsp;&nbsp;
				<a  href="#" id="btndelete" customdata='.$accountsingle->userid.' class="text-danger" data-toggle="modal" data-target="#delete"><i data-toggle="tooltip" data-placement="top" title='. trans('lang.delete').' class="ti-close"></i> </a>';
